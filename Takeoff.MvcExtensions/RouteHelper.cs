...
namespace Mediascend.Web
{
    ...
    {
        #region new, reflection-based exact routing
       ...
       // public const string MatchPositiveInteger = @"\d{1,10}";
        ...
        public static void RegisterRoutes(RouteCollection routes, Type appType, string area, bool registerGenericRoute)
        {
            routes.IgnoreRoute("{resource}.axd/{*pathInfo}");
            var controllers = appType.Assembly.GetTypes().Where(x => x.IsClass && !x.IsAbstract && x.IsSubclassOf(typeof(Controller)) && x.Name.EndsWith("Controller"));
            var actionMethods = controllers.SelectMany(type => type.GetMethods(BindingFlags.Instance | BindingFlags.Public).Where(m => m.ReturnType.Equals(typeof(ActionResult)) && Attribute.GetCustomAttribute(m, typeof(NonActionAttribute)) == null));
            string baseUrl = string.IsNullOrEmpty(area) ? string.Empty : area.EndWith("/");
            string namePrefix = area == null ? string.Empty : area;

            //home page
            routes.MapRouteLowerCase(namePrefix + "Index", baseUrl, new { controller = "Root", action = "Index" });


            actionMethods.Where(m => m.HasAttribute<RootUrlAttribute>() || m.DeclaringType.HasAttribute<RootUrlAttribute>()).Each((m, i) =>
            {
                var controllerName = m.DeclaringType.Name.EndWithout("Controller");
                var customName = (ActionNameAttribute)Attribute.GetCustomAttribute(m, typeof(ActionNameAttribute));
                var actionName = customName == null ? m.Name : customName.Name;
                routes.MapRouteLowerCase(namePrefix + "root" + i.ToInvariant(), baseUrl + actionName, new { controller = controllerName, action = actionName });
                routes.MapRouteLowerCase(namePrefix + "rootPlusId" + i.ToInvariant(), baseUrl + actionName + "/{id}", new { controller = controllerName, action = actionName, id = UrlParameter.Optional });
            });

            //for posting to a controller.  Instead of Index, Create action will be used.  Note the generic route handles teh Index action
            routes.MapRouteLowerCase(namePrefix + "Create", baseUrl + "{controller}/Create", new { action = "Create" });//handles Create GET requests.  Also can take POST requests for now.  Preferred way is to post to controller default.  Put this before one below so Url.Action works right for links and stuff.
            routes.MapRouteLowerCase(namePrefix + "CreateElse", baseUrl + "{controller}", new { action = "Create" }, new { httpMethod = new HttpMethodConstraint("POST") });

            var restIndex = 0;

            //sub controllers, so you can have urls like /projects/112/comments/52/edit
            controllers.Where(c => c.HasAttribute<SubControllerAttribute>()).Each((controllerType, index) =>
                {
                    foreach (SubControllerAttribute subController in Attribute.GetCustomAttributes(controllerType, typeof(SubControllerAttribute)))
                    {
                        var controllerName = controllerType.Name.EndWithout("Controller");
                        var urlRoot = (baseUrl.EndWith("/") + subController.BaseUrl.StartWithout("/").EndWith("/")).StartWithout("/");
                        if (!subController.UrlContainsControllerName)
                        {
                            urlRoot += controllerName.EndWith("/");//added actual controller name instead of"{controller}" because Url.Action wasn't working
                        }
                        routes.MapRouteLowerCase(namePrefix + "Sub" + index.ToInvariant() + "Rest", urlRoot + "{id}/{action}", new { controller = controllerName, action = "Details" }, new { id = new IdConstraint(), action = "details|edit|delete" });
                        routes.MapRouteLowerCase(namePrefix + "Sub" + index.ToInvariant() + "Generic", urlRoot + "{action}/{id}", new { controller = controllerName, action = "Index", id = UrlParameter.Optional });

                        foreach (var m in actionMethods.Where(m => m.DeclaringType == controllerType && Attribute.GetCustomAttribute(m, typeof(RestActionAttribute)) != null))
                        {
                            var customName = (ActionNameAttribute)Attribute.GetCustomAttribute(m, typeof(ActionNameAttribute));
                            var actionName = customName == null ? m.Name : customName.Name;
                            routes.MapRouteLowerCase(namePrefix + "rest" + restIndex.ToInvariant(), baseUrl + "{controller}/{id}/{action}", null, new { id = new IdConstraint(), controller = controllerName, action = actionName });
                            restIndex++;
                        }
            ...
            foreach (var m in actionMethods.Where(m => Attribute.GetCustomAttribute(m, typeof(RestActionAttribute)) != null))
            {
                var controllerName = m.DeclaringType.Name.EndWithout("Controller");
                var customName = (ActionNameAttribute)Attribute.GetCustomAttribute(m, typeof(ActionNameAttribute));
                var actionName = customName == null ? m.Name : customName.Name;
            ...
            //old urls.  put these last so action.url doesn't return old urls
            var oldUrlIndex = 0;
            foreach (var m in actionMethods.Where(m => Attribute.GetCustomAttributes(m, typeof(OldUrlAttribute)).HasItems()))
            {
                var controllerName = m.DeclaringType.Name.EndWithout("Controller");
                var customName = (ActionNameAttribute)Attribute.GetCustomAttribute(m, typeof(ActionNameAttribute));
                foreach (OldUrlAttribute oldUrl in Attribute.GetCustomAttributes(m, typeof(OldUrlAttribute)))
                {
                    var actionName = customName == null ? m.Name : customName.Name;
                    routes.MapRouteLowerCase(namePrefix + "oldurl" + oldUrlIndex.ToInvariant(), oldUrl.RouteSafeUrl, new { controller = controllerName, action = actionName });
                    oldUrlIndex++;
                }
            }

            //urls that take an id then an action, so things like Productions/234 or Productions/234/Edit are possible
            routes.MapRouteLowerCase(namePrefix + "Rest", baseUrl + "{controller}/{id}/{action}", new { action = "Details" }, new { id = new IdConstraint(), action = "details|edit|delete" });

            if ( registerGenericRoute)
                RegisterGenericRoute(routes, area);
        }

        public static void RegisterGenericRoute(RouteCollection routes, string area)
        {
//everything else.  
            string namePrefix = area ?? string.Empty;
            string baseUrl = string.IsNullOrEmpty(area) ? string.Empty : area.EndWith("/");
            routes.MapRouteLowerCase(namePrefix + "Generic", baseUrl + "{controller}/{action}/{id}",
        ...
        {
            Route route = new MadRoute(url, new MvcRouteHandler())
            {
        ...
        public class MadRoute : Route
        {
            public MadRoute(string url, IRouteHandler routeHandler)
                : base(url, routeHandler) { }
            public MadRoute(string url, RouteValueDictionary defaults, IRouteHandler routeHandler)
                : base(url, defaults, routeHandler) { }
            public MadRoute(string url, RouteValueDictionary defaults, RouteValueDictionary constraints, IRouteHandler routeHandler)
                : base(url, defaults, constraints, routeHandler) { }
            public MadRoute(string url, RouteValueDictionary defaults, RouteValueDictionary constraints, RouteValueDictionary dataTokens, IRouteHandler routeHandler) : base(url, defaults, constraints, dataTokens, routeHandler) { }
            public override VirtualPathData GetVirtualPath(RequestContext requestContext, RouteValueDictionary values)
            {
                //this block of code allows for a complex object to get its model included in the url parameters.  without this, the url would look like /controller/action?model=Takeoff.Data.SignupDetails or something.
                object model;
                if (values.TryGetValue("model", out model) && model != null)
                {
                    if ( Type.GetTypeCode(model.GetType()) == TypeCode.Object)
                    {
                        foreach (PropertyDescriptor descriptor in TypeDescriptor.GetProperties(model))
                        {
                            object propValue = descriptor.GetValue(model);
                            if (propValue != null)
                            {
                                values.Ensure(descriptor.Name, () => propValue);
                            }
                        }
                    }
                    values.Remove("model");
